...
private-key = ["6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV","5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3"]
producer-name = initu
plugin = io::producer_plugin
plugin = io::chain_api_plugin
plugin = io::account_history_plugin
plugin = io::account_history_api_plugin"""
...
def stageScenario(stagedNodeInfos):
    ...
    for stagedNodeInfo in stagedNodeInfos:
        configPath=os.path.join(stagingDir, "etc/io/node_%02d" % (count))
...
def myTest(transWillEnterBlock):
    ...
    try:
        ...
        if cluster.launch(pnodes, total_nodes, topo, delay) is False:
            error("Failed to stand up  cluster.")
            return False
        ...
        if walletMgr.launch() is False:
            error("Failed to stand up  walletd.")
            return False
