...
###############################################################
# nod_run_test
# --dump-error-details <Upon error print etc/io/node_*/config.ini and var/lib/node_*/stderr.log to stdout>
...
parser.add_argument("-h", "--host", type=str, help="%s host name" % (testUtils.Utils.ServerName),
                    default=LOCAL_HOST)
parser.add_argument("-p", "--port", type=int, help="%s host port" % testUtils.Utils.ServerName,
...
parser.add_argument("--dump-error-details",
                    help="Upon error print etc/io/node_*/config.ini and var/lib/node_*/stderr.log to stdout",
                    action='store_true')
...
try:
    ...
    # create accounts via io as otherwise a bid is needed 
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.ioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.ioAccount, stakedDeposit=0, waitForTransBlock=False)
    if transId is None:
        cmdError("%s create account" % (testeraAccount.name))
        errorExit("Failed to create account %s" % (testeraAccount.name))

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.ioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.ioAccount, stakedDeposit=5000)
    if transId is None:
        cmdError("%s create account" % (ClientName))
        errorExit("Failed to create account %s" % (currencyAccount.name))

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.ioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.ioAccount, waitForTransBlock=True)
    if transId is None:
        cmdError("%s create account" % (ClientName))
    ...
    contractDir="contracts/io.token"
    wastFile="contracts/io.token/io.token.wast"
    abiFile="contracts/io.token/io.token.abi"
    ...
    if not enableMongo:
        ...
        if abiName != "transfer" or abiActionName != "transfer" or abiType != "transfer":
            errorExit("FAILURE - get  account failed", raw=True)
    ...
    account=node.getAccount(defproduceraAccount.name)
    if account is None:
