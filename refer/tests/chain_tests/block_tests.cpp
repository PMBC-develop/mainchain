...
#include <io/testing/tester_network.hpp>
#include <io/chain/producer_object.hpp>

using namespace io;
using namespace io::chain;
using namespace io::testing;
...
uint32_t calc_exp_last_irr_block_num(const base_tester& chain, const uint32_t& head_block_num) {
   ...
   const auto max_reversible_rounds = _PERCENT(producers_size, config::percent_100 - config::irreversible_threshold_percent);
...
{ try {
      ...
      chain.push_action(name("io"), name("updateauth"), name("tester"), fc::mutable_variant_object()
              ("account", "tester")
              ("permission", "first")
              ("parent", "active")
...
// Test transaction_mroot matches with the specification in Github #1972 https://github.com/IO//issues/1972
...
{ try {
   ...
   validating_tester chain;
   // Finalize current block (which has set contract transaction for io)
   chain.produce_block();
...
BOOST_AUTO_TEST_CASE(producer_r1_key) { try {
   ...
   chain.push_action(N(io), N(setprods), N(io),
