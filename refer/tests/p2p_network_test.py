...
# io should have the same key as defproducera
io = copy.copy(defproduceraAccount)
io.name = "io"

Print("Info of each node:")
for i in range(len(hosts)):
    node = cluster.getNode(0)
    cmd="%s %s get info" % (testUtils.Utils.ClientPath, node.endpointArgs)
    trans = node.runCmdReturnJson(cmd)
    Print("host %s: %s" % (hosts[i], trans))


wastFile="contracts/io.system/io.system.wast"
abiFile="contracts/io.system/io.system.abi"
Print("\nPush system contract %s %s" % (wastFile, abiFile))
trans=node0.publishContract(io.name, wastFile, abiFile, waitForTransBlock=True)
if trans is None:
    Utils.errorExit("Failed to publish io.system.")
else:
...
try:
    ...
    for cmdInd in range(maxIndex):
        (transIdList, checkacct, expBal, errmsg) = module.execute(cmdInd, node0, testeraAccount, io)
