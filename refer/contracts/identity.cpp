...
#include <iolib/contract.hpp>
#include <iolib/dispatcher.hpp>
#include <iolib/vector.hpp>

namespace identity {
   using io::action_meta;
   using io::singleton;
   using io::key256;
   using std::string;
   using std::vector;
      ...
      public:
        ...
        using identity_base::identity_base;
         ...
         void create( const account_name creator, const uint64_t identity ) {
            ...
            io_assert( itr == t.end(), "identity already exists" );
            io_assert( identity != 0, "identity=0 is not allowed" );
                               ...
                               const vector<certvalue>& values )
         {
            ...
            idents_table t( _self, _self );
            io_assert( t.find( identity ) != t.end(), "identity does not exist" );
            ...
            for( const auto& value : values ) {
               ...
               if (value.confidence) {
                  io_assert(value.type.size() <= 32, "certrow::type should be not longer than 32 bytes");
                  ...
                  if (value.property == N(owner)) {
                     io_assert(sizeof(account_name) == value.data.size(), "data size doesn't match account_name size");
...
IO_ABI( identity::contract, (create)(certprop)(settrust) );
