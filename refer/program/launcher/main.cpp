...
#include <net/if.h>
#include <io/chain/genesis_state.hpp>
...
void
d_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string io_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), io_home(), instances(1) {}
};
...
struct launcher_def {
  ...
  producer_set_def producer_set;
   ...
   void assign_name (d_def &node, bool is_bios);
  ...
  void prep_remote_config_dir (d_def &node, host_def *host);
  void launch (d_def &node, string &gts);
  void kill (launch_modes mode, string sig_opt);
  pair<host_def, d_def> find_node(uint16_t node_num);
  vector<pair<host_def, d_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("skip-signature", bpo::bool_switch(&skip_transaction_signatures)->default_value(false), "nod does not require transaction signatures.")
    ("nod", bpo::value<string>(&d_extra_args), "forward nod command line argument(s) to each instance of nod, enclose arg in quotes")
    ...
    ("per-host",bpo::value<int>(&per_host)->default_value(0),"specifies how many nod instances will run on a single host. Use 0 to indicate all on one.")
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &d : binding.instances) {
          d.host = binding.host_name;
          d.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(d.p2p_port);
  ...
  char* erd_env_var = getenv ("IO_HOME");
...
void
launcher_def::assign_name (d_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.io_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      d_def d;
      assign_name(d, i == 0);
      aliases.push_back(d.name);
      d.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(d));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->io_home =
          (local_id.contains (lhost->host_name) || servers.default_io_home.empty()) ?
          erd : servers.default_io_home;
        host_ndx++;
      } // ph_count == 0

      d_def d;
      assign_name(d, do_bios);
      d.has_db = false;
      ...
      aliases.push_back(d.name);
      d.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(d));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->io_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->io_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->io_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->io_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->io_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = io::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = io::mongo_db_plugin\n";
  }
  if ( p2p == p2p_plugin::NET ) {
    cfg << "plugin = io::net_plugin\n";
  } else {
    cfg << "plugin = io::bnet_plugin\n";
  }
  cfg << "plugin = io::chain_api_plugin\n"
      << "plugin = io::history_api_plugin\n";
  cfg.close();
}
...
launcher_def::init_genesis () {
  bfs::path genesis_path = bfs::current_path() / "genesis.json";
   ...
   if (!src.good()) {
      ...
      io::chain::genesis_state default_genesis;
...
void
launcher_def::prep_remote_config_dir (d_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->io_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->io_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->io_home;

  cmd = "cd " + host->io_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->io_home
...
void
launcher_def::launch (d_def &instance, string &gts) {
  ...
  string dcmd = "programs/nod/nod ";
  if (skip_transaction_signatures) {
    dcmd += "--skip-transaction-signatures ";
  }
  if (!d_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +io::mongo_db_plugin");
       string args = std::regex_replace (d_extra_args,r,"");
  ...
  if( add_enable_stale_production ) {
    dcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  dcmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  if (gts.length()) {
    dcmd += " --genesis-timestamp " + gts;
  }

  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->io_home + "; nohup " + dcmd + " > "
    ...
    string cmd = "cd " + host->io_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else {
    cerr << "spawning child, " << dcmd << endl;

    bp::child c(dcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << dcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
...
#if 0
void
launcher_def::kill_instance(d_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, d_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, d_def>> node_list;
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const d_def& node = node_pair.second;
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.io_home + "; "
                 + "export IO_HOME=" + host.io_home + string("; ")
                 + "export IO_TN_NODE=" + node_num + "; "
                 + "./scripts/io-tn_bounce.sh";
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.io_home + "; "
                 + "export IO_HOME=" + host.io_home + "; "
                 + "export IO_TN_NODE=" + node_num + "; "
         + "export IO_TN_RESTART_CONFIG_DIR=" + node.config_dir_name + "; "
                 + "./scripts/io-tn_down.sh";
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the io-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the io-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the io-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << io::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( host_def,
            (genesis)(ssh_identity)(ssh_args)(io_home)
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (io_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_io_home) (ssh) )
