/**
 *  @file
 *  @copyright defined in io/LICENSE.txt
 */
#include <appbase/application.hpp>

#include <io/chain_plugin/chain_plugin.hpp>
#include <io/http_plugin/http_plugin.hpp>
#include <io/history_plugin.hpp>
#include <io/net_plugin/net_plugin.hpp>
#include <io/producer_plugin/producer_plugin.hpp>
...
{
   try {
      app().set_version(io::nod::config::version);
      app().register_plugin<history_plugin>();

      auto root = fc::app_path();
      app().set_default_data_dir(root / "io/nod/data" );
      app().set_default_config_dir(root / "io/nod/config" );
      if(!app().initialize<chain_plugin, http_plugin, net_plugin, producer_plugin>(argc, argv))
         return INITIALIZE_FAIL;
      initialize_logging();
      ilog("nod version ${ver}", ("ver", io::utilities::common::itoh(static_cast<uint32_t>(app().version()))));
      ilog("io root is ${root}", ("root", root.string()));
      app().startup();
      app().exec();
