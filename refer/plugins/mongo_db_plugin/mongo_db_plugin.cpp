...
public:
   ...
   static abi_def _abi; // cached for common use
                ...
                const chain::action& msg)
  {
     ...
     try {
        ...
        try {
           ...
           elog("Unable to convert  JSON to MongoDB JSON: ${e}", ("e", e.what()));
           elog("   JSON: ${j}", ("j", json));
        }
     } catch (fc::exception& e) {
...
// For now providing some simple account processing to maintain _balance
void mongo_db_plugin_impl::update_account(const chain::action& msg) {
   ...
   if (msg.name == transfer) {
      ...
      abi_serializer abis;
      auto io_account = find_account(accounts, msg.account);
      auto abi = fc::json::from_string(bsoncxx::to_json(io_account.view()["abi"].get_document())).as<abi_def>();
      abis.set_abi(abi);
      ...
      asset from_balance = asset::from_string(from_account.view()["_balance"].get_utf8().value.to_string());
      asset to_balance = asset::from_string(to_account.view()["_balance"].get_utf8().value.to_string());
      ...
      update_from << "$set" << open_document << "_balance" << from_balance.to_string()
                  << "updatedAt" << b_date{now}
                  << close_document;
      document update_to{};
      update_to << "$set" << open_document << "_balance" << to_balance.to_string()
                << "updatedAt" << b_date{now}
                << close_document;
...
void mongo_db_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         "The queue size between nod and MongoDB plugin thread.")
         ("mongodb-uri,m", bpo::value<std::string>(),
         "MongoDB URI connection string, see: https://docs.mongodb.com/master/reference/connection-string/."
               " If not specified then plugin is disabled. Default database '' is used if not specified in URI.")
         ;
}
...
void mongo_db_plugin::plugin_initialize(const variables_map& options)
{
   if (options.count("mongodb-uri")) {
      ...
      wlog("io::mongo_db_plugin configured, but no --mongodb-uri specified.");
