 ...
 */
#include <io/wallet_plugin/wallet_manager.hpp>
...
std::string wallet_manager::create(const std::string& name) {
   ...
   if (fc::exists(wallet_filename)) {
      _THROW(chain::wallet_exist_exception, "Wallet with name: '${n}' already exists at ${path}", ("n", name)("path",fc::path(wallet_filename)));
   }
   ...
   if(io_key.size())
      wallet->import_key(io_key);
   wallet->lock();
   wallet->unlock(password);
   
   // If we have name in our map then remove it since we want the emplace below to replace.
   // This can happen if the wallet file is removed while -walletd is running.
...
void wallet_manager::open(const std::string& name) {
   ...
   if (!wallet->load_wallet_file()) {
      _THROW(chain::wallet_nonexistent_exception, "Unable to open file: ${f}", ("f", wallet_filename.string()));
   }

   // If we have name in our map then remove it since we want the emplace below to replace.
   // This can happen if the wallet file is added while -walletd is running.
   auto it = wallets.find(name);
   if (it != wallets.end()) {
      wallets.erase(it);
   }
...
flat_set<public_key_type> wallet_manager::get_public_keys() {
   check_timeout();
   _ASSERT(!wallets.empty(), wallet_not_available_exception, "You don't have any wallet!");
   ...
   _ASSERT(!is_all_wallet_locked, wallet_locked_exception, "You don't have any unlocked wallet!");
   return result;
}
...
void wallet_manager::lock(const std::string& name) {
   ...
   if (wallets.count(name) == 0) {
      _THROW(chain::wallet_nonexistent_exception, "Wallet not found: ${w}", ("w", name));
   }
...
void wallet_manager::unlock(const std::string& name, const std::string& password) {
   ...
   auto& w = wallets.at(name);
   if (!w->is_locked()) {
      _THROW(chain::wallet_unlocked_exception, "Wallet is already unlocked: ${w}", ("w", name));
      return;
   }
   w->unlock(password);
}
...
void wallet_manager::import_key(const std::string& name, const std::string& wif_key) {
   ...
   auto& w = wallets.at(name);
   if (w->is_locked()) {
      _THROW(chain::wallet_locked_exception, "Wallet is locked: ${w}", ("w", name));
   }
...
wallet_manager::sign_transaction(const chain::signed_transaction& txn, const flat_set<public_key_type>& keys, const chain::chain_id_type& id) {
   ...
   for (const auto& pk : keys) {
      ...
      if (!found) {
         _THROW(chain::wallet_missing_pub_key_exception, "Public key not found in unlocked wallets ${k}", ("k", pk));
      }
   }
