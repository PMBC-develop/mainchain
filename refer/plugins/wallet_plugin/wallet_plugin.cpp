 ...
 */
#include <io/wallet_plugin/wallet_plugin.hpp>
...
void wallet_plugin::set_program_options(options_description& cli, options_description& cfg) {
   cfg.add_options()
         ...
         ("io-key", bpo::value<std::string>(),
          "io key that will be imported automatically when a wallet is created.")
         ;
}
...
void wallet_plugin::plugin_initialize(const variables_map& options) {
   ...
   if (options.count("io-key")) {
      std::string io_wif_key = options.at("io-key").as<std::string>();
      wallet_manager_ptr->set_io_key(io_wif_key);
   }
}
} // namespace io
