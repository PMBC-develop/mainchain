 ...
 */
#include <io/chain_plugin/chain_plugin.hpp>
#include <io/chain/fork_database.hpp>
#include <io/chain/block_log.hpp>
#include <io/chain/exceptions.hpp>
#include <io/chain/authorization_manager.hpp>
#include <io/chain/producer_object.hpp>
#include <io/chain/config.hpp>
#include <io/chain/types.hpp>
#include <io/chain/wasm_interface.hpp>
#include <io/chain/resource_limits.hpp>
#include <io/chain/reversible_block_object.hpp>

#include <io/chain/io_contract.hpp>

#include <io/utilities/key_conversion.hpp>
#include <io/utilities/common.hpp>
#include <io/chain/wast_to_wasm.hpp>
...
using namespace io;
using namespace io::chain;
using namespace io::chain::config;
using namespace io::chain::plugin_interface;
...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         ("wasm-runtime", bpo::value<io::chain::wasm_interface::vm_type>()->value_name("wavm/binaryen"), "Override default WASM runtime")
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      genesis_state gs; // Check if IO_ROOT_KEY is bad
   } catch( const fc::exception& ) {
      elog( "IO_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
            ("root_key", genesis_state::io_root_key) );
      throw;
   }
   ...
   if( options.count("extract-genesis-json") ||  options.at("print-genesis-json").as<bool>() ) {
      ...
      _THROW( extract_genesis_state_exception, "extracted genesis state from blocks.log" );
   }
   ...
   if( options.at("delete-all-blocks").as<bool>() ) {
      ...
      _THROW( fixed_reversible_db_exception, "fixed corrupted reversible blocks database" );
   }
...
read_only::get_info_results read_only::get_info(const read_only::get_info_params&) const {
   ...
   return {
      io::utilities::common::itoh(static_cast<uint32_t>(app().version())),
      db.get_chain_id(),
      db.head_block_num(),
...
abi_def get_abi( const controller& db, const name& account ) {
   ...
   _ASSERT(code_accnt != nullptr, chain::account_query_exception, "Fail to retrieve account for ${account}", ("account", account) );
   abi_def abi;
...
string get_table_type( const abi_def& abi, const name& table_name ) {
   ...
   _ASSERT( false, chain::contract_table_query_exception, "Table ${table} is not specified in the ABI", ("table",table_name) );
}
...
read_only::get_table_rows_result read_only::get_table_rows( const read_only::get_table_rows_params& p )const {
   ...
   _ASSERT( false, chain::contract_table_query_exception,  "Invalid table type ${type}", ("type",table_type)("abi",abi));
}
...
vector<asset> read_only::get_currency_balance( const read_only::get_currency_balance_params& p )const {
   ...
   walk_table<key_value_index, by_scope_primary>(p.code, p.account, N(accounts), [&](const key_value_object& obj){
      _ASSERT( obj.value.size() >= sizeof(asset), chain::asset_type_exception, "Invalid data on table");

      asset cursor;
      fc::datastream<const char *> ds(obj.value.data(), obj.value.size());
      fc::raw::unpack(ds, cursor);

      _ASSERT( cursor.get_symbol().valid(), chain::asset_type_exception, "Invalid asset");
...
fc::variant read_only::get_currency_stats( const read_only::get_currency_stats_params& p )const {
   ...
   uint64_t scope = ( io::chain::string_to_symbol( 0, boost::algorithm::to_upper_copy(p.symbol).c_str() ) >> 8 );

   walk_table<key_value_index, by_scope_primary>(p.code, scope, N(stat), [&](const key_value_object& obj){
      _ASSERT( obj.value.size() >= sizeof(read_only::get_currency_stats_result), chain::asset_type_exception, "Invalid data on table");
...
static fc::variant get_global_row( const database& db, const abi_def& abi, const abi_serializer& abis ) {
   ...
   _ASSERT(table_type == read_only::KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table global", ("type",table_type));

   const auto* const table_id = db.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple(N(io), N(io), N(global)));
   _ASSERT(table_id, chain::contract_table_query_exception, "Missing table global");
   ...
   _ASSERT(it != kv_index.end(), chain::contract_table_query_exception, "Missing row in table global");
...
read_only::get_producers_result read_only::get_producers( const read_only::get_producers_params& p ) const {
   const abi_def abi = get_abi(db, N(io));
   const auto table_type = get_table_type(abi, N(producers));
   const abi_serializer abis{ abi };
   _ASSERT(table_type == KEYi64, chain::contract_table_query_exception, "Invalid table type ${type} for table producers", ("type",table_type));

   const auto& d = db.db();
   const auto lower = name{p.lower_bound};

   static const uint8_t secondary_index_num = 0;
   const auto* const table_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple(N(io), N(io), N(producers)));
   const auto* const secondary_table_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple(N(io), N(io), N(producers) | secondary_index_num));
   _ASSERT(table_id && secondary_table_id, chain::contract_table_query_exception, "Missing producers table");
...
fc::variant read_only::get_block(const read_only::get_block_params& params) const {
   ...
   } _RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${block_num_or_id}", ("block_num_or_id", params.block_num_or_id))

   _ASSERT( block, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num_or_id));
...
void read_write::push_transaction(const read_write::push_transaction_params& params, next_function<read_write::push_transaction_results> next) {
   ...
   try {
      ...
      } _RETHROW_EXCEPTIONS(chain::packed_transaction_type_exception, "Invalid packed transaction")
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   const auto& code_account = db.db().get<account_object,by_name>( N(io) );
   //const abi_def abi = get_abi( db, N(io) );
   abi_def abi;
...
read_only::abi_json_to_bin_result read_only::abi_json_to_bin( const read_only::abi_json_to_bin_params& params )const try {
   ...
   _ASSERT(code_account != nullptr, contract_query_exception, "Contract can't be found ${contract}", ("contract", params.code));

   abi_def abi;
   if( abi_serializer::to_abi(code_account->abi, abi) ) {
      abi_serializer abis( abi );
      auto action_type = abis.get_action_type(params.action);
      _ASSERT(!action_type.empty(), action_validate_exception, "Unknown action ${action} in contract ${contract}", ("action", params.action)("contract", params.code));
      try {
         result.binargs = abis.variant_to_binary(action_type, params.args);
      } _RETHROW_EXCEPTIONS(chain::invalid_action_args_exception,
                                "'${args}' is invalid args for action '${action}' code '${code}'. expected '${proto}'",
                                ("args", params.args)("action", params.action)("code", params.code)("proto", action_abi_to_variant(abi, action_type)))
   } else {
      _ASSERT(false, abi_not_found_exception, "No ABI found for ${contract}", ("contract", params.code));
   }
   return result;
